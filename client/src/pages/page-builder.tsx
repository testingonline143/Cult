...
import { ChevronLeft, Plus, Trash2, Eye, EyeOff, ArrowUp, ArrowDown, Copy, Check, ExternalLink, Globe, Loader2, Calendar, MapPin, X, Pencil, LayoutGrid, List, Columns, Save } from "lucide-react";
...
export default function PageBuilder() {
  ...
  const profileMutation = useMutation({
    mutationFn: async (data: { shortDesc?: string; schedule?: string; location?: string }) => {
        ...
        <div className="rounded-2xl p-4 space-y-3" style={{ background: "var(--warm-white)", border: "1.5px solid var(--warm-border)" }}>
          ...
          {profileDirty && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-bold text-white disabled:opacity-50"
              style={{ background: "var(--terra)" }}
